const cards = [
	...
	},
	{
		...
		type: "TRAP",
		trapType: "CONTINUOUS",
		description: "When a Spell/Trap Card, or monster effect, is activated while you control a \"roid\" Fusion Monster: Negate the activation, then send all cards with the same name as that card from the activating player's Deck and Extra Deck to the GY. You can banish this card from the GY, then target 1 \"roid\" monster in your GY; add it to your hand. You can only use each effect of \"Emergeroid Call\" once per turn."
	},
	{
		id: 31,
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function updateFilterVisibilityByType() {
		const selectedTypes = Array.from(selectors.type)
	...
	function filterAndDisplay() {
		const searchText = form
			.querySelector('input[name="searchText"]')
			.value
			.trim()
			.toLowerCase();
		...
		const matchValue = value => {
			...
			return exactMatchChecked
				? value === searchText
				: value.includes(searchText);
		};

		const matchedCards = cards.filter(card => {
			if (selectedTypes.length && !selectedTypes.includes(card.type)) {
				return false;
			}
			...
			if (
				selectedMonsterTypes.length &&
				(!card.monsterType || !selectedMonsterTypes.includes(card.monsterType))
			) {
				return false;
			}
			...
			if (
				...
				(!card.subtype || !card.subtype.some(s => selectedSubtypes.includes(s)))
			) {
				return false;
			}
			...
			if (
				...
				(!card.spellType || !selectedSpellTypes.includes(card.spellType))
			) {
				return false;
			}

			if (
				selectedTrapTypes.length &&
				(!card.trapType || !selectedTrapTypes.includes(card.trapType))
			) {
				return false;
			}
			...
			if (
				card.rank !== undefined &&
				(card.rank < levelMin || card.rank > levelMax)
			) {
				return false;
			}

			if (
				card.linkRating !== undefined &&
				(card.linkRating < linkMin || card.linkRating > linkMax)
			) {
				return false;
			}
			...
			if (
				card.atk !== undefined &&
				(card.atk < atkMin || card.atk > atkMax)
			) {
				return false;
			}
			...
			if (
				card.def !== undefined &&
				(card.def < defMin || card.def > defMax)
			) {
				return false;
			}
		...
		if (!matchedCards.length) {
			...
			resultsSection.innerHTML =
				`<p style="font-size: 2rem; text-align: center;">No cards found matching your search.</p>`;
		...
		matchedCards.forEach(card => {
			...
			let html =
				`<h3>${card.name}</h3>` +
				`<img src="${card.image}" alt="${card.name}" loading="lazy" width="200" height="200">`;
			...
			if (card.spellType) {
				const spellTypeText = Array.isArray(card.spellType)
					? card.spellType.join(", ")
					: card.spellType;
			...
			if (card.trapType) {
				const trapTypeText = Array.isArray(card.trapType)
					? card.trapType.join(", ")
					: card.trapType;
			...
			saveBtn.addEventListener("click", () => {
				...
				const index = savedCards.findIndex(
				...
				if (index === -1) {
					...
					ariaLiveRegion.textContent =
						`${card.name} saved to your library.`;
				} else {
					...
					ariaLiveRegion.textContent =
						`${card.name} removed from your library.`;
				}
	...
	[
		...selectors.type,
		...selectors.attribute,
		...selectors.monsterType,
		...selectors.subtype,
		...selectors.spellType,
		...selectors.trapType,
		...Object.values(rangeInputs),
